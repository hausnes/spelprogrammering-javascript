...
var racket_venstre = {
    ...
    "hoyde" : 50,
    "farge" : "Black",
    "xpos" : 15, //15 piksler fra høyre kant
    "ypos" : bane.hoyde / 2, //Omtrent på midten vertikalt
    "yretning" : 0, //Hvilken vei racketen beveger seg
    "yfart" : 5 //Piksler racketen flytter seg
};
...
function tegnRacketVenstre() {
    ctx.fillStyle = racket_venstre.farge; //Farge på racketen
    ctx.fillRect(racket_venstre.xpos, racket_venstre.ypos, racket_venstre.bredde, racket_venstre.hoyde); //Tegner opp racketen
    if (racket_venstre.ypos <= 0 && racket_venstre.yretning === -1) {
        return; //Avslutter funksjonen
    }
    if (racket_venstre.ypos + racket_venstre.hoyde >= bane.hoyde && racket_venstre.yretning === 1) {
        return; //Avslutter funksjonen
    }
    racket_venstre.ypos = racket_venstre.ypos + racket_venstre.yfart * racket_venstre.yretning;
}
...
function sjekkOmBallTrefferRacketVenstre() {
    var ballenErTilVenstre = ball.xpos + ball.radius < racket_venstre.xpos; // returnerer true eller false
    var ballenErtilHoyre = ball.xpos - ball.radius > racket_venstre.xpos + racket_venstre.bredde;
    var ballenErOver = ball.ypos + ball.radius < racket_venstre.ypos;
    var ballenErUnder = ball.ypos - ball.radius > racket_venstre.ypos + racket_venstre.hoyde;
    ...
    if(!ballenErTilVenstre && !ballenErtilHoyre && !ballenErOver && !ballenErUnder) {
        ...
        poengVenstre = poengVenstre + 1;
        hdnPoeng.innerHTML = "Poeng: " + poengVenstre;
        if(poeng > localStorage.rekord) {
            localStorage.rekord = poeng;
            hdnRekord.innerHTML = "Ny rekord: " + poeng;
            hdnRekord.style.color = "DarkGreen";
...
function gameLoop() {
    ...
    sjekkOmBallTrefferRacketVenstre();
...
document.onkeyup = function(evt) {
    ...
    if(tastekode === 40 && racket.yretning === 1) {
        racket.yretning = 0;}
    if(tastekode === 81 && racket_venstre.yretning === -1) {
        racket_venstre.yretning = 0;}
    if(tastekode === 65 && racket_venstre.yretning === 1) {
        racket_venstre.yretning = 0;}
}